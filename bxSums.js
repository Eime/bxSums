...
(function() {
    ...
    if (myJQuery(".main-kanban-column").length) {
        var
          $columns = myJQuery(".main-kanban-column-body"),
          allLoaded = _.after($columns.length, function () {
              window.setInterval(calcEstimations, 2000);
          });

        // Wir muessen alle Columns scrollen, um sicherzugehen dass alle Karten geladen sind:
        myJQuery(".main-kanban-column-body").each(function () {
            var $col = myJQuery(this);
            $col.scrollTop(50000);
            _.delay(function () {
                $col.scrollTop(0);
                allLoaded();
...
function calcEstimations() {
    myJQuery(".main-kanban-column").each(function () {
        var
            $column = myJQuery(this),
            $target = $column.find(".customBxSums"),
            $cards = $column.find(".main-kanban-item"),
            totalDonesAll = 0,
        ...
        _.each(persons, function ($responsible, responsible) {
            if (totalDones[responsible] || totalEstimations[responsible]) {
                ...
                myJQuery("<li>")
                    ...
                    .bind("click", function () {
                        filterResponsible(myJQuery(this), responsible);
                    })
                    .toggleClass("filtered", responsible === $column.attr("rel"))
                    .append(
                        myJQuery("<div>").text(
...
function filterResponsible ($li, responsible) {
    var
        $column = $li.parents(".main-kanban-column"),
        $items = $column.find(".main-kanban-item"),
        curFilter = $column.attr("rel"),
        newFilter = responsible;

    if (!responsible || curFilter === responsible) {
        newFilter = null;
    }
    ...
    myJQuery(".customBxSums li").removeClass("filtered");
